...
def test_find_by_properties_pass():
    ...
    flexmock(artifact).should_receive("query_artifactory").and_return(mock_response_good)
...
def test_find_by_pattern_pass():
    ...
    mock_json_good = json.dumps({"repoUri": mock_repo_uri, "files": mock_file_list})
    mock_response_good = flexmock(status_code=200, text=mock_json_good)
    
    flexmock(artifact).should_receive("query_artifactory").and_return(mock_response_good)
    flexmock(artifact).should_receive("get_repositories").and_return({"key":"mock-repo-local"})
...
def test_find_by_pattern_fail():
    ...
    flexmock(artifact).should_receive("query_artifactory").and_return(mock_response_bad)
